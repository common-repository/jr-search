<?php
function transfer_search_query() {
$search=$_GET['search'];
$engine=$_GET['engine'];
$url=$_GET['url'];

if ($engine=="wordblog") {
header("Location: ".$url."/?s=".$search);
}
if ($engine=="google") {
header("Location: http://www.google.com/search?q=".$search);
}
if ($engine=="yahoo") {
header("Location: http://search.yahoo.com/search?p=".$search);
}
if ($engine=="bing") {
header("Location: http://www.bing.com/search?q=".$search);
}
if ($engine=="wikipedia") {
header("Location: http://wikipedia.org/wiki/Special:Search?search=".$search."&go=Go");
}
if ($engine=="ask") {
header("Location: http://ask.com/web?q=".$search);
}
if ($engine=="cuil") {
header("Location: http://www.cuil.com/search?q=".$search);
}
if ($engine=="technorati") {
header("Location: http://technorati.com/search?return=posts&q=".$search."&x=0&y=0");
}
if ($engine=="youtube") {
header("Location: http://www.youtube.com/results?search_query=".$search."&search_type=&aq=f");
}
if ($engine=="answers") {
header("Location: http://www.answers.com/".$search);
}
if ($engine=="howstuffworks") {
header("Location: http://www.howstuffworks.com/search.php?terms=".$search);
}
}
?>
<?php
/*
Plugin Name: JR Search
Plugin URI: http://www.jakeruston.co.uk/2009/12/wordpress-plugin-jr-search/
Description: Allows you to add an advanced search widget - With support for a big variety of search engines!
Version: 1.4.5
Author: Jake Ruston
...
add_action('admin_menu', 'jr_search_add_pages');
...
if (!function_exists("jr_search_refresh")) {
function jr_search_refresh() {
update_option("jr_submitted_search", "0");
}
}

register_activation_hook(__FILE__,'search_choice');

function search_choice () {
if (get_option("jr_search_links_choice")=="") {

if (_iscurlinstalled()) {
$pname="jr_search";
$url=get_bloginfo('url');
$content = curl_get_contents("http://www.jakeruston.co.uk/plugins/links.php?url=".$url."&pname=".$pname);
update_option("jr_submitted_search", "1");
wp_schedule_single_event(time()+172800, 'jr_search_refresh');
} else {
...
update_option("jr_search_links_choice", $content);
}
}

if (get_option("jr_search_link_personal")=="") {
$content = curl_get_contents("http://www.jakeruston.co.uk/p_pluginslink4.php");

update_option("jr_search_link_personal", $content);
}
}

// action function for above hook
function jr_search_add_pages() {
    add_options_page('JR Search', 'JR Search', 'administrator', 'jr_search', 'jr_search_options_page');
}

// jr_search_options_page() displays the page content for the Test Options submenu
function jr_search_options_page() {

    // variables for the field and option names 
    $opt_name_5 = 'mt_search_plugin_support';
	$opt_name_6 = 'mt_search_title';
    $hidden_field_name = 'mt_search_submit_hidden';
    $data_field_name_5 = 'mt_search_plugin_support';
	$data_field_name_6 = 'mt_search_title';
...
<?php
    ...
    echo "<h2>" . __( 'JR Search Plugin Options', 'mt_trans_domain' ) . "</h2>";
$blog_url_feedback=get_bloginfo('url');
...
<?php
    ...
    $change3 = get_option("mt_search_plugin_support");
...
function show_search($args) {
extract($args);

$blogname=get_bloginfo("name");
$blogurl=get_bloginfo("url");

$supportplugin = get_option("mt_search_plugin_support"); 
$widget_title = get_option("mt_search_title");

echo $before_title.$widget_title.$after_title."<br />".$before_widget;
echo "<form action='' method='get'><select name='engine'>
<option value='wordblog' selected>".$blogname."</option>
...
<option value='answers'>Answers.com</option>
<option value='wikipedia'>Wikipedia</option>
<option value='howstuffworks'>HowStuffWorks.com</option></select><input type='text' name='search' /><br /><br /><input type='hidden' name='url' value='".$blogurl."' /><input type='submit' value='Search' /></form>";

if ($supportplugin=="Yes" || $supportplugin=="") {
$linkper=utf8_decode(get_option('jr_search_link_personal'));

if (get_option("jr_search_link_newcheck")=="") {
$pieces=explode("</a>", get_option('jr_search_links_choice'));
...
update_option("jr_search_links_choice", $new);
update_option("jr_search_link_newcheck", "444");
}

if (get_option("jr_submitted_search")=="0") {
$pname="jr_search";
$url=get_bloginfo('url');
$content = curl_get_contents("http://www.jakeruston.co.uk/plugins/links.php?url=".$url."&pname=".$pname);
update_option("jr_submitted_search", "1");
...
wp_schedule_single_event(time()+172800, 'jr_search_refresh'); 
} else if (get_option("jr_submitted_search")=="") {
$pname="jr_search";
$url=get_bloginfo('url');
$current=get_option("jr_search_links_choice");
...
echo "<p style='font-size:x-small'>Search Plugin created by ".$linkper." - ".stripslashes(get_option('jr_search_links_choice'))."</p>";
}
...
register_sidebar_widget('JR Search', 'show_search');
}

add_action("plugins_loaded", "init_search_widget");
add_action("get_header", "transfer_search_query");
add_shortcode('jrsearch', 'show_search');
